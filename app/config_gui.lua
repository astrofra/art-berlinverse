...
function config_gui(default_res_x, default_res_y, open_vr_enabled, language, use_sprite_fx)
    ...
    if loaded_config then
        if loaded_config.default_res_x ~= nil then default_res_x = loaded_config.default_res_x end
    ...
    local lang_preset = (array_find(language_list, language) or 1) - 1
    ...
    local res_preset = (array_find(res_list_str, tostring(default_res_x) .. "x" .. tostring(default_res_y)) or 4) - 1 -- 960x720 by default
